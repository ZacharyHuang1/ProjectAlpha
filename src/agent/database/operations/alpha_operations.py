...
def _alpha_id(alpha_data: Dict[str, Any]) -> str:
    """Best-effort extraction of the alpha's stable identifier.

    Schema compatibility note:
    - New pipeline uses: alpha_id / expression / description
    - Older code paths used: alphaID / expr / desc
    This function lets both formats persist correctly.
    """
    return (
        alpha_data.get("alpha_id")
        or alpha_data.get("alphaID")
        or alpha_data.get("id")
        or str(uuid.uuid4())
    )


def _alpha_expression(alpha_data: Dict[str, Any]) -> str:
    return alpha_data.get("expression") or alpha_data.get("expr") or ""


def _alpha_description(alpha_data: Dict[str, Any]) -> str:
    return alpha_data.get("description") or alpha_data.get("desc") or ""
            ...
            for alpha_data in state_values["seed_alphas"]:
                ...
                if not existing:
                    alpha = Alpha(
                        ...
                        description=_alpha_description(alpha_data),
                        code=(alpha_data.get("dsl") or alpha_data.get("code") or ""),
                    )
                    session.add(alpha)
