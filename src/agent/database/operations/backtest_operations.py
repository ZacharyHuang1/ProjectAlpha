...
def _lookup_alpha_key(sota_alpha: Dict[str, Any]) -> Any:
    """Extract a key we can use to find the Alpha row.

    Preference order:
    1) numeric DB id ("id") if it's an int
    2) stable alpha identifier ("alpha_id" / "alphaID" / "id")
    """
    raw_id = sota_alpha.get("id")
    if isinstance(raw_id, int):
        return raw_id
    return sota_alpha.get("alpha_id") or sota_alpha.get("alphaID") or raw_id
...
def save_backtest_results(
    ...
    try:
        for sota_alpha in state_values["sota_alphas"]:
            ...
            alpha_key = _lookup_alpha_key(sota_alpha)
            alpha_query = session.query(Alpha).filter_by(thread_id=thread_id)
            if isinstance(alpha_key, int):
                alpha = alpha_query.filter(Alpha.id == alpha_key).first()
            else:
                alpha = alpha_query.filter(Alpha.alpha_id == str(alpha_key)).first()
