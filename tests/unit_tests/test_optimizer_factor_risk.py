...
def test_factor_risk_penalty_shifts_weights() -> None:
    ...
    # One-factor model: only A loads the factor, so it should be down-weighted.
    loadings = pd.DataFrame({"f1": [1.0, 0.0, 0.0, 0.0]}, index=scores.index)
    factor_cov = np.array([[1.0]], dtype=float)
    idio_var = pd.Series(0.0, index=scores.index)

    cm = OptimizerCostModel(
        risk_aversion=10.0,
        risk_model="factor",
